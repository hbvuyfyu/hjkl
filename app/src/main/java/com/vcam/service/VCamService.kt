...
import com.vcam.service.CameraInjectorDebug
...
class VCamService : Service() {
    ...
    private var injector: CameraInjectorDebug? = null
    ...
    private suspend fun startInjection(
        ...
        try {
            injector = CameraInjectorDebug(
                context       = this,
                mediaPath     = mediaPath,
                isVideo       = isVideo,
                targetPackage = targetPackage
            )
            DebugLogger.section("بدء Injection Service")
            DebugLogger.log("الهدف: $targetName")
            injector?.start()
            DebugLogger.success("Injection بدأ بنجاح! ✓")
            ...
            DebugLogger.error("خطأ في بدء Injection", e)
    ...
    private fun switchToSlot(slot: Int) {
        ...
        serviceScope.launch {
            try {
                DebugLogger.section("تبديل الحقل إلى $slot")
                // Try vcplax hot-swap first (no interruption)
                val proxy = VcplaxEngine.getProxy()
                if (proxy != null && VcplaxEngine.isRunning) {
                    DebugLogger.log("استخدام hot-swap عبر VcplaxEngine")
                    proxy.switchSource(path, if (isVideo) 2 else 1)
                    DebugLogger.success("تم التبديل بنجاح عبر VcplaxEngine")
                } else {
                    // Restart injection with new media
                    DebugLogger.log("إعادة تشغيل Injection برسالة جديدة...")
                    injector?.stop()
                    injector = CameraInjectorDebug(
                    ...
                    injector?.start()
                    DebugLogger.success("تم بدء Injection برسالة جديدة")
                }
                DebugLogger.endSection()
                updateNotification("VCam — Slot $slot Active",
                    if (isVideo) "🎬 فيديو" else "📷 صورة $slot")
            } catch (e: Exception) {
                DebugLogger.error("خطأ في تبديل الحقل", e)
            }
        }
    }
